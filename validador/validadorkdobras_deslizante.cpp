...
aux = 2;//quantas previsoes a frente?????
int aux2 = 0;
    ...
    for (i=0; i<numeroIteracoes; i++){
        ...
        iniciotreino = aux2+aux;
        fimtreino = iniciotreino + tam_janela_deslizante - 1;
        inicioteste = fimtreino + 1;
        fimteste = inicioteste + aux; //teste para ALLL11. treina com 16, testa 10. treina 16...
        aux2++;
...
vector< vector< float > > ValidadorKFoldDeslizante::executarExperimento3( Treinador &treinador,
    ...
    tam_treino = corpus.pegarQtdSentencas() - 22;// temp diego
    ...
    outsaidaarq2 << "../outputs/classificacao/#corpus_" << ativo << "_" << jan << "_" << alg << ".txt";
    vetCorpus[1]->gravarArquivo(outsaidaarq2.str());
